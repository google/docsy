...
[f3128285][], mapped to where each is covered. This is the objective "is
everything covered, in the right place?" snapshot and the entry point for each
refresh — add a row per new commit and route it.

All 18 commits in range are squash-merged PRs (one commit per PR), so the
...
- **Hugo bumps** ([#2647][], [#2648][], [#2649][]): [#2647][] also carries the
  **breaking** minimum-Hugo bump (0.146.0 → 0.158.0), language-API deprecation
  cleanup, and a deprecation-notice build guard; [#2649][] migrates docsy.dev
  off the deprecated global `imaging.quality` and regenerates md-output goldens.
