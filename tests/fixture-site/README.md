...
**theme-generated** — written by that build (`npm run update:markup-goldens`)
and re-compared against a **fresh build on every test run**, so a committed
golden cannot silently drift from what the theme actually generates: divergence
is a red test, and the refreshed golden's diff is the review artifact. The same
fixture feeds the visual suite (`../visual/`).
