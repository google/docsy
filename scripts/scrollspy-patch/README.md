...
The `_test:full:pre` script calls `_prepare` and `is:clean` to ensure no
...
- Run `npm run _test:full:pre` to ensure that the patch file and runtime patch
  code are up to date. If the command fails, review the changes and update the
  patch file manually, as described below.
...
- Rerun `npm run _test:full:pre` to ensure that the updated patch file works,
  and to update RUNTIME_PATCHER_JS.
- Commit changes.
