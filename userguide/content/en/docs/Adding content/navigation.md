...
### Add manual links to section menu
 ...
 If a page has defined the parameter `manualLink` in its metadata (front matter) the generated link in the section menu for this page will be replaced by the value of `manualLink`. Optional you can also set a link title and target with the parameters `manualLinkTitle` and `manualLinkTarget`. 
