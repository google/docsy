...
Docsy's `docs` layout gives you a left sidenav with an autogenerated nested
entries based on your `docs` file structure. Each standalone page or subsection
`_index.md` or `_index.html` page in the `docs/` directory appears as a sidenav
top-level entry, using the link name and `weight` metadata from the page or
index.
