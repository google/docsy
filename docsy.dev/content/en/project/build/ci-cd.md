...
The default GitHub Actions [`test` workflow][test-workflow] runs `npm run
ci:test` on Ubuntu (formatting, markdownlint, site build, link checks, Markdown
golden tests). It does **not** run AFDocs; add a step to your own workflow if you
want continuous scoring (for example against a preview URL).

[AFDocs]: https://afdocs.dev/
[agent-docs-config]: https://github.com/google/docsy/blob/main/docsy.dev/agent-docs.config.yml
[test-workflow]: https://github.com/google/docsy/blob/main/.github/workflows/test.yaml
