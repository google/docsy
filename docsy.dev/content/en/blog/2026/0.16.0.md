...
support makes them unnecessary: `theme/package.json` declares Bootstrap, Font
Awesome, and the PostCSS toolchain, and [`hugo mod npm pack`][hugo-npm-pack]
delivers them to your project. This also retires a generated `theme/go.mod`, a
module-sync script, and a Bootstrap `rfs` vendor workaround ([#2668][]).
...
dependency set drifts. The PostCSS tools (`autoprefixer`, `postcss-cli`) now
come from Docsy, so a Hugo-module site no longer installs them separately.

Sites that install Docsy as an npm package or via clone or submodule are
unaffected: they continue to get Bootstrap and Font Awesome through Docsy's
`postinstall`.
