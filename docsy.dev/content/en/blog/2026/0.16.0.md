...
This also retires a generated `theme/go.mod`, a module-sync script, and a
Bootstrap `rfs` vendor workaround ([#2668][]).
...
Docsy now runs `postCSS` only for RTL languages (which need it for `rtlcss`) or
when your site provides a project-root `postcss.config.{js,mjs,cjs}`. Non-RTL
sites without such a config no longer need a PostCSS toolchain at all and can
drop `autoprefixer`, `postcss`, and `postcss-cli` from their dependencies.
