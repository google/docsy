...
For the generic update steps (Docsy theme, Node, Hugo, and [your theme
overrides][overrides]), follow [Update Docsy][], which reflects the install
paths introduced by this release's [theme folder move](#theme-folder-actions);
apply that section's config changes as part of the update. For this release,
use:[^vers-note]
...
version 0.15.0 at the restored path (for Hugo modules:
`hugo mod get github.com/google/docsy@v0.15.0`). If you switched to the
[registry package](#npm-registry), reinstall from GitHub instead:
