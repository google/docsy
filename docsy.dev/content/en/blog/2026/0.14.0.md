...
- {{% _param BREAKING %}} Rename `td-offset-anchor` to
  `td-anchor-no-extra-offset`. See [Implementation
  notes][heading-aliases-impl-notes]. **Applies if** your site uses
  `td-offset-anchor`; for example, if you override `blocks/lead.html`,
  `blocks/section.html`, or `layouts/community/list.html`.
...
- {{% _param BREAKING %}} Using [blocks/cover] with Markdown content? Call it
  with Hugo's shortcode [Markdown call syntax][]: `{{%/* */%}}`, otherwise it
  might not render content correctly. **Applies if** you use `blocks/cover` in
  `.html` content files (Markdown in the body is not processed).
- (Optional) If your navbar overlays the cover, add the `td-below-navbar` helper
  class to your `blocks/cover` calls to position the cover below the navbar. For
  details, see [Below-navbar height adjustment][].
...
- `blocks/cover`: use the [Markdown call syntax](#blocks-cover-actions) if you
  use Markdown content, and add the `td-below-navbar` helper class to position
  the cover below the navbar, if needed.
...
#### Advanced overrides (optional) {#advanced-overrides}

If your project overrides Docsy templates or shortcodes, review these updated
files and port changes as needed:
