...
- {{% _param FAS_LG thumbtack warning %}}
  <span>**[Pinned script versions](#script-pins)**: Mermaid, KaTeX, and markmap
  no longer float on the CDN's `latest`</span>
...
- **Install and defaults**:
  ...
  - [Default script-dependency versions pinned](#script-pins): Mermaid, KaTeX,
    and markmap no longer resolve the CDN's `latest`
...
- No Hugo change this release: the [version list](#upgrade) is otherwise
  unchanged too.
- Optionally skim:
  - [Default script-dependency versions pinned](#script-pins)
  - [Other notable changes](#other-notable-changes), and
...
### Actions {#script-pins-actions}

**Applies if** you want a different version of one of these dependencies.

- Set [`params.mermaid.version`][mermaid-docs],
  [`params.katex.version`][katex-docs], or
  [`params.markmap.version`][markmap-docs] in your site config.
...
- [ ] Mermaid diagrams render at the [pinned version](#script-pins).
