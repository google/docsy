...
- **If you set `params.markmap.enable` or `params.disable_click2copy_chroma`**:
  ...
  warning. Migrate to the registry, then remove the old parameter: for MarkMap,
  the [MarkMap guide][markmap-guide] (which also says why the old parameter must
  go); for the copy button, `click-to-copy: false` under
  [`params.docsy.plugins`][ug-plugins].
