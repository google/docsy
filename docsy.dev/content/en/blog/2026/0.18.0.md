...
button. Disabled integrations ship no JavaScript, MarkMap loads only on pages
that use it, and every plugin script is fingerprinted with [SRI][] ([#2789][]).
...
- **If you produce MarkMap markup other than through a `markmap` fence on the
  page itself** (raw HTML, your own render hook or shortcode, Docsy's `tab` or
  `readfile code="true"` shortcodes with `lang=markmap`, or a fence in content
  pulled in with `.Content`), the page-gated script no longer sees it: set
  `pageGate: ''` on the markmap entry to load it site-wide.
...
- **If your build has no network access or restricts Hugo's remote fetches**,
  ...
  at build time instead of by the browser ([MarkMap guide][markmap-guide]). To
  build offline, override the companion partial `scripts/plugins/markmap.html`
  to serve a copy you host.
