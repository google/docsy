...
- **If you enable the `print` output**: its pages no longer load tab persistence
  or registry-enabled MarkMap (page-gated scripts never reached print output;
  Mermaid and KaTeX never did).
...
- **If your site config has a `params.docsy` key of its own**, rename it: Docsy
  ...
  turns the theme plugins off (with a build warning).
