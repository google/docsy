...
- **No action needed** if you don't override the affected files. The legacy
  `params.markmap.enable` still works for this release cycle (with a build
  warning) and keeps its pre-0.18 site-wide loading; to enable MarkMap the new
  way, see the [user guide][markmap-guide]. Remove the legacy param once you
  register MarkMap: the registry entry supersedes it.
