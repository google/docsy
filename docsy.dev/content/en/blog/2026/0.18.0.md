...
registry for loading scripts without layout overrides, and converts its first
three features onto it: MarkMap, tab-pane persistence, and the code-block copy
button. Disabled integrations ship no JavaScript, MarkMap and tab persistence
load only on pages that use them, and every plugin script is fingerprinted with
[SRI][] ([#2789][]).
...
- **If you enable the `print` output for sections**: a printed section no longer
  loads tab persistence or registry-enabled MarkMap for the pages it gathers
  (their page flags don't reach it; Mermaid and KaTeX have always had this gap).
...
- **If you produce MarkMap or tab-persistence markup other than through a
  `markmap` fence or the `tabpane` shortcode** (raw HTML, your own render hook
  or shortcode, Docsy's `tab` or `readfile` shortcodes with `lang=markmap`), the
  page-gated scripts no longer see it: set `pageGate: ''` on the plugin's
  registry entry to load it site-wide.
...
- **If you override the tabpane shortcode**, refresh your copy: page gating keys
  on a Store flag (`hasTabs`) that only the current shortcode sets, so a stale
  copy silently stops loading tab persistence.
...
- **If your build has no network access or restricts Hugo's remote fetches**,
  ...
  at build time instead of by the browser ([MarkMap guide][markmap-guide]).
