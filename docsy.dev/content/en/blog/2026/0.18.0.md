...
- **No action needed** if you don't override the affected files. The legacy
  ...
  warning, and keeping its pre-0.18 site-wide loading; to enable MarkMap the new
  way, see the [user guide][markmap-guide].
...
- **If you override the tabpane shortcode or markmap code-block rendering**,
  ...
  that only the current templates set -- a stale copy silently stops loading the
...
- **If your site restricts Hugo's remote fetches** (`security.http`), allow
  ...
  build time and serve it from your site, as Mermaid and KaTeX builds already
  do.
