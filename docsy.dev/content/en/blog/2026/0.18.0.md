...
- **If you override a converted file**, [review your override against the new
  ...
  - `assets/js/base.js`, `search.js`, `offline-search.js`, and `plantuml.js`,
    plus `assets/js/plugins/markmap.js` and
...
- **If you produce MarkMap markup other than through a `markmap` fence on the
  page itself, or print sections that contain one**: the page-gated script no
  longer sees it. Set `pageGate: ''` on the markmap entry; for the affected
  authoring paths, see the [MarkMap guide][markmap-guide].
...
- **If you override `scripts.html`**, a stale copy fails the build: 0.18
  decomposed it into a dispatcher over `scripts/*.html` sub-partials; take the
  current file and re-apply your change to the sub-partial it belongs to.
